                        ...
                        <el-collapse-item :title="ci.ciName" v-for="(ci, ciIndex) in ciReportArr" :key="ci.ciId">
                            <el-row style="color: #888;">
                                <el-col :span="12" style="box-sizing: border-box; padding: 6px 10px;"
                                    ...
                                    <span style="margin-right:  5px; vertical-align: top;">{{ cidr.name }}</span>

                                    <el-input style="width: 26%; 
                                    margin-right: 2px;" size="small" :placeholder="cidr.name" v-if="cidr.type != 4"
                                        v-model="ciReportArr[ciIndex].cidrList[cidrIndex].value"
                                        @blur="cidrCheckByBlur(ciIndex, cidrIndex)" />
                                    <el-input style="width: 80%;" tpye="textarea" :rows="2" :placeholder="cidr.name"
                            ...
                            <el-button style="margin-top: 8px;" type="primary" @click="updateCiDetailedReport(ciIndex)"
                            v-if="orders.state==1">{{
                    ...
                    <el-card class="box-card" style="margin-top: 20px;">
                        <template #header>
                            <div class="card-header">
                                ...
                                @click="updateOrdersState" v-if="orders.state==1">体检套餐总检结果报告归档</el-button>
                            </div>
                        </template>
                        <div>
                                ...
                                <el-table-column label="操作" width="150">
                                    <template #default="scope">
                                        <el-button type="text" size="small"
                                            @click="toUpdateOverallResult(scope.row)" v-if="orders.state==1">更新</el-button>
                                        <el-button type="text" size="small"
                                            @click="removeOverallResult(scope.row)" v-if="orders.state==1">删除</el-button>
                                    </template>
                                </el-table-column>
                            </el-table>
                            <el-form ref="formRef" :model="overallResultForm" style="margin-top: 20px;" label-width="100px"
                            v-if="orders.state==1">
                    ...
                    <el-dialog v-model="dialogVisible" title="总检结论项更新" width="60%">
                        <span>
                            <el-form :model="updateOverallResultForm" label-width="120px">
                                ...
                                <el-form-item label="总检结论内容">
                                    <el-input v-model="updateOverallResultForm.content" type="textarea" :rows="3"/>
                                </el-form-item>
                                <el-form-item>
...
export default {
    ...
    setup() {
        ...
        function cidrCheckByBlur(ciIndex, cidrIndex) {
            ...
            //判断type属性1.数值范围验证型，2.数值相等验证型
            if (cidr.type == 1) {
                if (!(cidr.value == null || cidr.value == '') && (cidr.value < cidr.minrange || cidr.value > cidr.maxrange)) {
                    cidr.isError = 1;
                } else {
                    cidr.isError = 0;
                }
            } else if (cidr.tpye == 2) {
                if (!(cidr.value == null || cidr.value == '') && (cidr.value != cidr.normalValue)) {
                    cidr.isError = 1;
                } else {
                    cidr.isError = 0;
                }
        ...
        function updateCiDetailedReport(ciIndex) {
            ...
            for (let i = 0; i < cidrArr.length; i++) {
                ...
                if (cidrArr[i].type == 1 && parseFloat(cidrArr[i].value).toString() == 'NaN') {
        ...
        function updateOrdersState(){
            if(!confirm('总检结论报告归档前,请务必确认是否所有检查项都正确!')){
                return ;
            }
            axios
